...
├── data/                         # Real labeled WMFT recordings (42 trials, 17 classes)
│   ├── <classID>_<task><trial>.txt
│   ├── manifest.csv              # path,label index over the recordings above
│   └── wmftclassifier.m          # Original MATLAB reference implementation
├── examples/
...
## Train On The Bundled Real Data

`data/` contains 42 real labeled WMFT recordings (1-4 trials across all 17 movement classes) with a ready-to-use `data/manifest.csv`:
...
On the current augmented dataset (420 synthetic recordings from 42 real trials, 10 variants each), this gives:
